...
export async function GET() {
  ...
  return NextResponse.json({ success: true, reservations, stats: await getStats() });
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (markAllRead) {
      await markAllReservationsRead();
      return NextResponse.json({ success: true, reservations: await getReservations(), stats: await getStats() });
    }

    if (id && durum) {
      const updated = await updateReservationStatus(id, durum);
      if (!updated) {
        return NextResponse.json({ error: "Rezervasyon bulunamadı" }, { status: 404 });
      }
      return NextResponse.json({ success: true, reservation: updated, stats: await getStats() });
    }
