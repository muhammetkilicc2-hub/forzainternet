...
export async function getComputers(): Promise<PC[]> {
  const dbData = await readData<PC[]>("computers", "bilgisayar_state.json");
  if (dbData && Array.isArray(dbData) && dbData.length > 0) return dbData;
  
  const initial = createInitialPcList();
  // İlk çalıştırmada veritabanı boşsa ilk listeyi yazdıralım
  await writeData("computers", "bilgisayar_state.json", initial);
  return initial;
}
...
export async function getReservations(): Promise<Rezervasyon[]> {
  const dbData = await readData<Rezervasyon[]>("reservations", "rezervasyon_state.json");
  return Array.isArray(dbData) ? dbData : [];
}

export async function createReservation(data: Omit<Rezervasyon, "id" | "olusturuldu" | "durum">): Promise<Rezervasyon> {
  ...
  await writeData("reservations", "rezervasyon_state.json", list);
  await updateComputerStatus(data.masaId, "rezerve");

  return newRez;
}

export async function updateReservationStatus(id: string, durum: "confirmed" | "rejected"): Promise<Rezervasyon | null> {
  const list = await getReservations();
  ...
  if (durum === "confirmed") {
    await updateComputerStatus(rez.masaId, "kullanimda");
  } else if (durum === "rejected") {
    await updateComputerStatus(rez.masaId, "bos");
  }
...
export async function markAllReservationsRead(): Promise<void> {
  const list = await getReservations();
  list.forEach((r) => { r.okundu = true; });
  await writeData("reservations", "rezervasyon_state.json", list);
}
